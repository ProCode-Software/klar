...
func Run(c *argparse.Parser) {
	...
	clearProjFlag, clearGlobalFlag := c.Flag("project"), c.Flag("global")
	clearProjCache := clearProjFlag.Bool()
	clearGlobalCache := clearGlobalFlag.Bool()
	switch {
	case !clearProjCache && !clearGlobalCache:
		cli.Failure(
			"'--project' and '--global' can't both be disabled, otherwise there's no work to do!",
		)

	// If one is set, disable the other implicitly
	case clearGlobalFlag.Set && !clearProjFlag.Set:
		clearProjCache = false
	case clearProjFlag.Set && !clearGlobalFlag.Set:
...
var Flags = argparse.NewParser().
	BoolFlag("project", "Clear project cache (in '.klar/cache') only", true, "p").
	BoolFlag("global", "Clear global cache (for all the user's projects) only", true, "g")
