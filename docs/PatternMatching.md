...
word := 'hello'
when word[0]!! {
    // Matches if the string is a single letter in the range
    'A'...'Z' -> {}
    'a'...'z' -> {}
}
...
when map {
    ...
    #{a, b: 2} -> {}
    // Matches a map containing `a` and `b` with values `1` and `2`
    // respectively, and exactly 1 more key
    #{a: 1, b: 2, _}
    ...
    // is a compile error.
    #{...} -> {}
    // Matches a map with exactly 2 keys
    #{_, _} -> {}
