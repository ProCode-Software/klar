...
1. **Major version** - Indicates versions that introduce features that break compatibility with older major versions.
...
4. **Revision** - Indicates a release that does not change functionality or add new features, such as updates to metadata, documentation, or formatting.
...
5. `dev` - Usually used for nightly builds and versions built from the main branch of the source code.
...
- `*` - Allow any version
- `latest [tag]` - Require the latest version available, including versions with tag `[tag]`.
    - If `latest beta` is used, this matches the latest tagged with `beta` or anything more stable, whichever is newer.
    - If `[tag]` is not provided, this matches the latest stable version, disallowing any tagged version.
